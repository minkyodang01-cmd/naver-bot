...
def load_csv_rows(path):
    try:
        ...
        print("CSV LOAD ERROR:", path, str(e))
        return []
...
# 질문키 = 질문
# 응답유형 = 실제 답변 텍스트
# 응답텍스트 = text / text_button
# 따라서 FAQ는 텍스트 응답으로 고정 처리
# =========================
...
for row in load_csv_rows(FAQ_PATH):
    ...
    answer = safe_str(row.get("응답유형", "")) or safe_str(row.get("답변", ""))
...
# PRODUCT CSV 로드
# 헤더:
# 구분, 품명, 온도, 재질, 특징, 제품사진, 도면사진, 순서, 사용여부
# =========================
...
product_data.sort(key=lambda x: (safe_str(x["구분"]), x["순서"], safe_str(x["품명"])))


# =========================
# OEM CSV 로드
# 헤더:
# OEM, HANMI P/N, OEM P/NO, DESCRIPTION, Temp, Type, PDF 링크, 순서
# =========================
...
oem_data.sort(key=lambda x: (safe_str(x["OEM"]), x["순서"], safe_str(x["HANMI P/N"])))
...
def build_reply_target(source):
    return {
        "userId": safe_str((source or {}).get("userId", "")),
        "channelId": safe_str((source or {}).get("channelId", "")),
    }
...
def send_request(target, body):
    token = get_token()

    channel_id = safe_str((target or {}).get("channelId", ""))
    user_id = safe_str((target or {}).get("userId", ""))
    ...
    if channel_id:
        ...
        raise Exception("회신 대상(userId/channelId)이 없습니다.")
...
def send_text_message(target, text):
    body = {
        "content": {
            "type": "text",
            "text": safe_str(text)
    ...
    send_request(target, body)


def send_button_template(target, content_text, actions):
    valid_actions = [a for a in actions if a]
    if not valid_actions:
        send_text_message(target, content_text)
        return
...
def send_flex_message_groups(target, category, valid_rows):
...
def send_flex_spec_pages(target, category):
    rows = get_rows_by_category(category)

    if not rows:
        send_text_message(target, f"{category} 목록이 없습니다.")
        return
    ...
    if not valid_rows:
        send_text_message(target, f"{category} 유효한 데이터가 없습니다.")
        return

    send_flex_message_groups(target, category, valid_rows)
...
def send_product_group_menu(target):
    groups = get_product_groups()
    if not groups:
        send_text_message(target, "제품 데이터가 없습니다.")
        return
    ...
    send_button_template(target, "제품구분을 선택하세요.", actions)


def send_product_items_by_group(target, group_name):
    rows = get_products_by_group(group_name)

    if not rows:
        send_text_message(target, f"{group_name} 제품이 없습니다.")
        return
    ...
    lines.append("")
    lines.append("원하는 품명을 입력하세요.")
    send_text_message(target, "\n".join(lines))
...
def send_product_flex(target, row):
    bubbles = build_product_bubbles(row)

    if not bubbles:
        send_text_message(target, safe_str(row.get("품명", "")) or "제품 정보가 없습니다.")
        return
...
# OEM 기능
# =========================
def build_oem_bubble(row):
    oem_name = safe_str(row.get("OEM", "")) or "-"
    ...
    sub_lines = []
    if oem_pno:
        sub_lines.append(f"OEM P/NO : {oem_pno}")
    if desc:
        sub_lines.append(desc)
    if temp:
        sub_lines.append(f"Temp : {temp}")
    if type_value:
        sub_lines.append(f"Type : {type_value}")
...
def send_oem_flex(target):
    if not oem_data:
        send_text_message(target, "OEM 데이터가 없습니다.")
        return

    bubbles = [build_oem_bubble(row) for row in oem_data]
    bubble_groups = chunk_list(bubbles, MAX_BUBBLES_PER_MESSAGE)

    for idx, group in enumerate(bubble_groups, start=1):
        body = {
            "content": {
                "type": "flex",
                "altText": f"OEM 목록 {idx}",
                "contents": {
                    "type": "carousel",
                    "contents": group
                }
...
# 메인 처리
# =========================
def handle_message(target, raw_msg):
    ...
    if msg_normalized in FAQ_NORMALIZED:
        send_text_message(target, FAQ_NORMALIZED[msg_normalized])
        return

    # PRODUCT 직접 품명 입력
    matched_product = find_product_by_name(raw_msg)
    if matched_product:
        send_product_flex(target, matched_product)
        return

    # PRODUCT 시작
    if msg_normalized == "제품":
        send_product_group_menu(target)
        return

    # PRODUCT 구분 선택
    if msg_upper.startswith("PRODG|"):
        group_name = raw_msg.split("|", 1)[1].strip()
        send_product_items_by_group(target, group_name)
        return

    # OEM
    if msg_normalized == "OEM":
        send_oem_flex(target)
        return

    # 기존 스펙
    if msg_normalized in ["스펙", "스팩", "SPEC"]:
        send_category_menu(target)
        return

    if msg_upper.startswith("CAT|"):
        category = msg_upper.split("|", 1)[1].strip().upper()
        if category in CATEGORY_LIST:
            send_flex_spec_pages(target, category)
            return

    if msg_upper in CATEGORY_LIST:
        send_flex_spec_pages(target, msg_upper)
        return

    # FAQ 유사어는 마지막
    similar_key = find_similar_faq_key(msg_normalized)
    if similar_key:
        send_text_message(target, FAQ_NORMALIZED[similar_key])
        return

    send_text_message(target, "원하시는 기능을 찾지 못했습니다.")
...
def bot():
    ...
    event_type = safe_str(req.get("type", ""))

    if event_type and event_type not in ["message", "postback"]:
        return "ok", 200

    target = build_reply_target(source)
    raw_msg = safe_str(content.get("text", ""))

    if not target.get("userId") and not target.get("channelId"):
        return "ok", 200
    ...
    try:
        handle_message(target, raw_msg)
    except Exception as e:
        print("HANDLE ERROR:", str(e))
        try:
            send_text_message(target, "처리 중 오류가 발생했습니다.")
